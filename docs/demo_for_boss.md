...
**Q: Why is BDD editing and Script viewing not yet in the web portal? (Darcy's April 1 item)**
A: Darcy's item #4 on April 1st asked: *"In the same Web portal, the process should progress to BDD and Script stage respectively, and display BDDs generated by AI, and allow Human to directly edit the BDD. After human modification, we should save it. Again, this will be used as Audit Trail."*

This is **not missing by oversight — it was deferred** to a later phase for three reasons:

**1. The BDD and Script pipelines are already functional (offline).**
The BDD pipeline (`run_bdd_pipeline()` in `pipelines.py`) and step registry (`compute_step_matches()` in `step_registry.py`) both work end-to-end via CLI. They generate `normalized_bdd.jsonl`, Gherkin `.feature` files, and step definition stubs. The BDD rendering engine (`bdd_export.py`) also auto-generates Ruby step definitions from `TEMPLATE_REGISTRY`. These are all complete.

**2. The review portal was intentionally scoped to the maker/checker feedback loop.**
The web portal (`review_session.py`) was built to solve a specific problem: getting human approval on AI-generated scenarios before they flow into downstream automation. That loop — maker → checker → human review → rewrite → checker again → report — is now complete and auditable. Extending it to BDD/Script editing adds a second stage progression which introduces new UX complexity (stage gates, state tracking across stages, editing BDD in-browser with validation).
...
- A Scripts tab in the same shell
