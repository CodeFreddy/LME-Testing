...
class StubProvider:
    ...
    def generate(self, system_prompt: str, user_prompt: str) -> ModelResponse:
        ...
        if role == "planner":
            ...
            payload = self._stub_scripts_response(user_prompt)
        elif role == "bdd":
    ...
    def _detect_role(self, system_prompt: str) -> str:
        ...
        if "you are the maker" in prompt_lower:
            return "maker"
        if "you are the checker" in prompt_lower:
            return "checker"
        if "you are the bdd" in prompt_lower:
            return "bdd"
        if "you are the planner" in prompt_lower:
            return "planner"
        if "behave python step implementation code" in prompt_lower or "one script per input step_id" in prompt_lower:
            return "scripts"
    ...
    def _extract_prompt_json_after_marker(self, user_prompt: str, marker: str) -> object:
        marker_pos = user_prompt.rfind(marker)
        if marker_pos == -1:
            return None
        text = user_prompt[marker_pos + len(marker):].lstrip()
    ...
    def _stub_maker_response(self, rule_ids: list[str], user_prompt: str) -> dict:
        ...
        req_map_match = re.search(r"Expected requirement_ids map:\s*(\{[^\n]*\})", user_prompt)
        if req_map_match:
            try:
                parsed_req_map = json.loads(req_map_match.group(1))
            except json.JSONDecodeError:
                parsed_req_map = {}
            for sid, values in parsed_req_map.items():
                if isinstance(values, list) and values:
                    req_ids[sid] = [str(value) for value in values]

        case_map_match = re.search(r"Expected required_case_types map:\s*(\{[^\n]*\})", user_prompt)
        if case_map_match:
            try:
                parsed_case_map = json.loads(case_map_match.group(1))
            except json.JSONDecodeError:
                parsed_case_map = {}
            for sid, values in parsed_case_map.items():
                if isinstance(values, list):
                    case_type_map[sid] = [str(value) for value in values]
                ...
                scenarios.append(
                    {
                        ...
                        "evidence": [
                            {
                                "atomic_rule_id": req_ids.get(sid, [f"R-{sid}"])[0],
                                "page": 1,
                                "quote": f"Evidence for {sid}.",
                            }
                        ],
                    }
                )
    ...
    def _stub_checker_response(self, rule_ids: list[str], user_prompt: str) -> dict:
        ...
        for match in re.finditer(r'"scenario_id"\s*:\s*"([^"]+)"', user_prompt):
            ...
            case_section = user_prompt[match.start(): match.start() + 1200]
            case_type_match = re.search(r'"case_type"\s*:\s*"([^"]+)"', case_section)
            if case_type_match:
                case_type_map[case_id] = case_type_match.group(1)
            ...
            results.append(
                {
                    ...
                    "case_type": case_type_map.get(case_id, "positive"),
    ...
    def _stub_scripts_response(self, user_prompt: str) -> dict:
        steps = self._extract_prompt_json_after_marker(user_prompt, "Input steps:")
        catalog = self._extract_prompt_json_after_marker(user_prompt, "API catalog:")
        if not isinstance(steps, list):
            steps = []
        endpoints = []
        if isinstance(catalog, dict) and isinstance(catalog.get("endpoints"), list):
            endpoints = [item for item in catalog["endpoints"] if isinstance(item, dict)]
        ...
        for item in steps:
            ...
            if step_type not in {"given", "when", "then"} or not step_id:
                continue
            ...
            scripts.append(
                {
                    ...
                    "code": code,
                    "notes": "Generated by StubProvider for local POC validation.",
                }
            )
